...
import { PrivacyLevel, Status, Urls } from './constants.js';
import mustache from 'mustache';
import { extractErrorMessage, suppressSensitiveInformation, sanitizeAndClean, replaceAll } from './util.js';
import { info } from '@actions/core';
/**
 * Fetches sponsors from the GitHub Sponsors API.
 */
export async function getSponsors(action) {
    try {
        info(`Fetching data from the GitHub API as ${action.organization ? 'Organization' : 'User'}… ⚽`);
        const query = `query { 
      ${action.organization
        ...
        throw new Error(`There was an error with the GitHub API request: ${suppressSensitiveInformation(extractErrorMessage(error), action)} ❌`, { cause: error });
    }
}
/**
 * Generates the sponsorship template.
 */
export function generateTemplate(response, action) {
    let template = ``;
     ...
     */
    const data = action.organization && response?.data?.organization
    ...
    if (sponsorshipsAsMaintainer) {
        ...
        if (action.includePrivate) {
             ...
             */
            filteredSponsors = filteredSponsors.filter((user) => user.privacyLevel !== PrivacyLevel.PRIVATE);
        }
        if (action.maximum > 0) {
        ...
        info(`Found ${filteredSponsors.length} matching ${filteredSponsors.length === 1 ? 'sponsor' : 'sponsors'}… ${filteredSponsors.length > 0 ? '🎉' : '😢'}`);
        /**
        ...
        filteredSponsors.map(({ sponsorEntity }) => {
            ...
            const sanitizedSponsorEntity = {
                websiteUrl: sanitizeAndClean(sponsorEntity.websiteUrl || sponsorEntity.url),
             ...
             */
            const safeTemplate = replaceAll(replaceAll(action.template, '{{{', '{{'), '}}}', '}}');
 ...
 */
export async function generateFile(response, action) {
    try {
        ...
        throw new Error(`There was an error generating the updated file: ${suppressSensitiveInformation(extractErrorMessage(error), action)} ❌`, { cause: error });
    }
}
